...
function ESX.SetFunction(fnName, fn)
    ...
    if fnNameType ~= "string" then ESX.Trace(("The function name (^3%s^7) passed in ^5ESX.SetFunction^7 in ^3%s^7 is not a valid string!"):format(fnName, lib.context), "error", true) return false
    elseif not isFnValid then ESX.Trace(("The function passed in ^5ESX.SetFunction^7 in ^3%s^7 is not a valid function!"):format(lib.context), "error", true) return false end

    if fnName == "SetFunction" then ESX.Trace(("Function ^2%s^7 of ESX ^1cannot^7 be overrided!"):format(fnName), "error", true) return false end
