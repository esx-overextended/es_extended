...
function ESX.SetFunction(fnName, fn)
    ...
    if fnNameType ~= "string" then print(("[^1ERROR^7] The function name (^3%s^7) passed in ^5ESX.SetFunction^7 in ^3%s^7 is not a valid string!"):format(fnName, lib.context)) return false
    elseif not isFnValid then print(("[^1ERROR^7] The function passed in ^5ESX.SetFunction^7 in ^3%s^7 is not a valid function!"):format(lib.context)) return false end
    ...
    if Config.EnableDebug then
        print(("[^5INFO^7] Setting function (^2%s^7) for ^2ESX^7 through ^5ESX.SetFunction^7 in ^3%s^7."):format(fnName, lib.context))
    end
